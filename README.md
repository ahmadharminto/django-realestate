...
```
CREATE DATABASE real_estate_prod;
```
...
```
GRANT ALL PRIVILEGES ON DATABASE real_estate_prod TO dbadmin;
```
...
```
# git clone https://github.com/yourgithubname/real_estate_project.git
```
...
# Local Settings Setup or using **secrets.template.json**
...
```
# gunicorn --bind 0.0.0.0:8000 real_estate.wsgi
```
...
```
# sudo nano /etc/nginx/sites-available/real_estate_project
```
...
```
# sudo ln -s /etc/nginx/sites-available/real_estate_project /etc/nginx/sites-enabled
```
...
### Edit /etc/nginx/sites-available/real_estate_project
